...
Description: This plugin takes a simple list of titles and quickly turns them into draft posts or pages.
Version: 1.1
		...
		if ( ! empty($titles)) :
			...
			foreach ( $titles as $title ) {
				...
				if ('post' == $_POST['bulk_post_type']) {
					if ($new_draft_id = self::create_draft($title, 'post')) {
						echo '<li>Created <a href="post.php?action=edit&post='.$new_draft_id.'">'.$title.'</a>'.PHP_EOL;
					}
				} else {
					if ($new_draft_id = self::create_draft($title, 'page')) {
			...
			if ('post' == $_POST['bulk_post_type']) {
				...
				echo '<p>All done! <a href="edit-pages.php?post_status=draft">See all drafts &raquo;</a></p>'.PHP_EOL;
			}
			
		endif;
	}
	
	private function create_draft($title = null, $type = 'post') {
		if ( ! empty($title)) {
			global $wpdb;
